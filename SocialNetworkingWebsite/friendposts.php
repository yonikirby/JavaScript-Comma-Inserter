...
$sql = "select * from userinfo where id='$myID'";
                       ...
                       $friend_cover_src = $_SESSION["friend_cover_src"] = $row['coverphotopath'];
                       $friend_profile_src = $_SESSION["friend_profile_src"] = $row['profilephotopath'];
                       $friend_firstname = $_SESSION["friend_firstname"] = $row['firstname'];
                       $friend_lastname = $_SESSION["friend_lastname"] = $row['lastname'];
                       $friend_bio = $_SESSION["friend_bio"] = stripslashes($row['bio']);
                       $friend_hobbies = $_SESSION["friend_hobbies"] = stripslashes($row['hobbies']);
...
<div id = "PostsCreatePost">
	<div id = "PostsCreatePostInner">
		<div id = "PostsCreatePostTop">
			...
			<div id = "PostsWhatsOnYourMind" onclick = "showCreatePost('<?php echo $myID; ?>', '<?php echo $friend_firstname; ?>')">
				&nbsp;&nbsp;Write something to <?php echo $friend_firstname; ?>...
			</div>
			
			
		</div>
		
		<div id = "PostsCreatePostAddPhotoDiv" onclick = "showCreatePost(); addPhoto();">
...
<script>
if (!isFriends) {
    $("#PostsCreatePost").css("display","none");
}
                    ...
                    <div class = "PostsOnePost" id = "PostsOnePost'.$postid.'">
                        <div class = "PostsOnePostInner">
                            ...
                            <div class = "LikeCommentShare" id = "LikeCommentShare'.$postid.'">
                                <div class = "LikeDiv ItemButton" data-wall_id = "'.$posteeid.'" data-post_author_id="'.$posterid.'" id = "LikeDiv'.$postid.'" data-fullname = "'.$posterfirstname.' '.$posterlastname.'">
    ...
    function generateComments($result, $parentID) {
        ...
        while ($row = mysqli_fetch_array($result)){  //Write First Level of Comments
                    ...
                    if ($row['parentid'] == $parentID){     
                            //echo '<script>alert("$parentID: '.$parentID.' and row[parentid] : '.$row['parentid'].'");</script>';
                            echo '
                            <script>
        ...
        foreach ($result as $row) { //Write Second Level of Comments
                    ...
                    if ($row['ultimateparentid'] == $parentID && $row['parentid'] != $parentID){ 
                        //echo '<script>alert("subcomment $parentID: '.$parentID.' and row[parentid] : '.$row['parentid'].'");</script>';
                        echo '
                        <script>
                        ...
                        */
                        ...
                        //$("#WriteReplyDiv'.$parentID.'").css("display", "block");
                        //$("#WriteReplyDiv'.$parentID.'").before(toInsert);
                        //$("#WriteReplyDiv'.$parentID.'").css("display", "none");
...
$(document).on('click', '.ReplyCommentLink', function(event){ //top level posts LIKE button clicked
            ...
            if ($( this ).data("commentstatus") == "firstlevelcomment") {
                    if ($("#WriteReplyDiv" + myNumber + ":visible").length == 0){//document.getElementById("#WriteReplyDiv" + myNumber).getAttribute("display");
                        //alert("got into click!");
                        
                        //$("#PostsPageOneComment" + myNumber).after("<br />");
                ...
                if ($("#WriteReplyDiv" + myParentID + ":visible").length == 0){//document.getElementById("#WriteReplyDiv" + myNumber).getAttribute("display");
                        //alert("got into click!");
                        
                        //$("#PostsPageOneComment" + myParentID).after("<br />");
...
$(".PostsPageMakeComment").on('click', function(e) {  
